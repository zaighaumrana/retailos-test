...
console.log(el.dataset.pId);
    ...
    if (client?.supabase_url && client?.supabase_anon) {
      try {
        const clientSb = createClient(client.supabase_url, client.supabase_anon);

console.log("CLIENT URL:", client.supabase_url);
console.log("SUSPENDED VALUE:", isSuspending);

const result = await clientSb
  .from("shop_config")
  .update({ suspended: isSuspending })
  .eq("id", 1);
